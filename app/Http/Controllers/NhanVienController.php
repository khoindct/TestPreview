...
class NhanVienController extends Controller
{
    ...
    public function login(Request $request) {
        ...
        $nhanvien = NhanVien::where('email', $email)->get();

        if (empty($nhanvien)) {
            return redirect('dangnhap')->with('tkthatbai', 'Tài khoản không tồn tại');
        } else {
            if (!Hash::check($password, $nhanvien[0]->password)) {
                return redirect('dangnhap')->with('mkthatbai', 'Mật khẩu không chính xác');
            } else {
